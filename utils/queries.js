...
const BonusCardDescription = (post) => {
   if (!post) return {};
   return {
      description: post.description,
   };
...
export async function queryPartners() {
   ...
   result.results[0].data.partners?.map((partner) => {
...
export async function queryBonusCardDescription() {
   const result = await Client().query(
      Prismic.Predicates.at("document.type", "bonus_card_description")
   );

   return BonusCardDescription(result.results[0]?.data);
}
